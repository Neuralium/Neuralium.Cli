...
```")]
        ...
        // -----------------------------------------------------------------------------		
        [Command(typeof(DynamicPeerOperation), LongName = nameof(DynamicPeerOperation), Description = "Allows you to dynamically add/remove/whitelist/blacklist a peer (Advanced Users) (bool)")]
        ...
        // -----------------------------------------------------------------------------		
        [Command(typeof(GenerateTestPuzzle), LongName = nameof(GenerateTestPuzzle), Description = "Generate a test puzzle (Expert Users) (string)")]
        GenerateTestPuzzle,
        // -----------------------------------------------------------------------------		
        [Command(typeof(GetBlockSizeAndHash), LongName = nameof(GetBlockSizeAndHash), Description = "Single block's size and hash (string)")]
        GetBlockSizeAndHash,
        // -----------------------------------------------------------------------------		
        [Command(typeof(GetCurrentOperatingMode), LongName = nameof(GetCurrentOperatingMode), Description = "Returns the current operations mode (int)")]
...
")]
        ...
        ReadAppSetting,
        // -----------------------------------------------------------------------------	
        [Command(typeof(GenericOperation), ShortName = "r", LongName = "run", Description = @"Run an operations by name, allows you to use the --jparams option. For example:
...
")]
   ...
   * ValidatorHttp = 3")]
    ...
    public class GenerateTestPuzzle : NamedOperation { }
...
")]
    ...
    public class DynamicPeerOperation : NamedOperation
    {
        [PositionalArgument(ArgumentFlags.Required, Position = 0, Description = "The Peer's IP address")]
        public string Ip { get; set; }
		
        [PositionalArgument(ArgumentFlags.Required, Position = 1, Description = "The Peer's port (use -1 or 33888 for default port)")]	
        public int Port { get; set; }
	    
        [PositionalArgument(ArgumentFlags.Required, Position = 2, Description = @"The operation to perform. Choices are
